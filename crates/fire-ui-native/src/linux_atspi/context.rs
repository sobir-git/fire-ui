...
use zbus::{connection::Builder, proxy::PropertyChanged, Connection};

use crate::linux_atspi::{
    adapter::{AdapterState, Callback, Message},
    atspi::{map_or_ignoring_recoverable_error, zbus_error_is_unrecoverable, Bus},
        ...
        *state = match entry.activation_handler.request_initial_tree() {
            Some(initial_state) => {
                let r#impl = AdapterImpl::with_wrapped_action_handler(
                    ...
                    Callback::new(entry.edits.clone()),
                    initial_state,
                    *is_window_focused,
                    *root_window_bounds,
    ...
    match message {
        ...
        Message::RegisterInterfaces {
            ...
            if let Some(bus) = atspi_bus {
                bus.register_interfaces(node, interfaces, edits).await?
            }
        }
